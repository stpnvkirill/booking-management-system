# ruff: noqa: DTZ007, DTZ011, RUF006, RUF001, RUF002
...
async def confirm_booking(callback: CallbackQuery, state: FSMContext):
    ...
    display_booking["status"] = STATUS_MAP.get(
        booking.get("status"),
        booking.get("status"),
    )
    ...
    await callback.message.edit_text(
        ...
        "Мы напомним вам перед началом 👍\n\n" + format_booking(display_booking),
    ...
    async def notify(updated_booking: dict):
        display_updated = updated_booking.copy()
        display_updated["status"] = STATUS_MAP.get(
            updated_booking.get("status"),
            updated_booking.get("status"),
        )

        await callback.message.answer(
            "🔔 Статус обновлён:\n\n" + format_booking(display_updated),
